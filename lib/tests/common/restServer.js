...
class RestServer {
    constructor() {
        ...
        this.listJobs = () => nock(`http://${this.testUri}`).get('/api/v2/jobs').reply(200, testJobList_1.testJobList);
        this.listJobsQuery = () => nock(`http://${this.testUri}`).get('/api/v2/jobs?username=core').reply(200, testJobList_1.testJobList);
