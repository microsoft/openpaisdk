...
const paiUserErrors_1 = require("../../src/commom/errors/paiUserErrors");
...
const unauthorizedToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6InlpeWkiLCJhcHBsaWNhdGlvbiI6ZmFsc2' +
        ...
        tests: [
            {
                ...
                operation: {
                    parameters: [
                        ...
                        {
                            type: 'raw',
                            value: clusters_json_1.default[0].password
                        }
                    ]
                }
            },
        ...
        tests: [
            {
                ...
                operation: {
                    parameters: [{
                            type: 'raw',
                            value: {
                                username: clusters_json_1.default[0].username,
                                email: 'new_email@test1.com'
                            }
                        }]
                }
                    ...
                    parameters: [
                        {
                            ...
                            value: {
                                username: clusters_json_1.default[0].username,
                                email: 'new_email@test2.com',
                                oldPassword: clusters_json_1.default[0].password,
                                newPassword: clusters_json_1.default[0].password
                            }
                        },
                        {
                            type: 'raw',
...
class CustomizedTestsClass {
    ...
    async getTokensWithUnauthorizedUser(test, operationResults) {
        ...
        try {
            ...
            chai_1.expect(err).to.be.an.instanceof(paiUserErrors_1.UnauthorizedUserError);
        }
    }
    async logoutWithCorrectToken(test, operationResults) {
        const client = new clients_1.AuthnClient({
            token: operationResults.beforeResults[0].token,
            https: true,
            rest_server_uri: clusters_json_1.default[0].rest_server_uri
        });
        const res = await client.basicLogout();
        chai_1.expect(res.message).to.be.eq('Logout successfully');
    }
    async logoutWithIncorrectToken(test, operationResults) {
        const client = new clients_1.AuthnClient({
            token: unauthorizedToken,
            https: true,
            rest_server_uri: clusters_json_1.default[0].rest_server_uri
        ...
        try {
            ...
            chai_1.expect(err.message).to.be.equal('Your token is invalid.');
            chai_1.expect(err).to.be.an.instanceof(paiUserErrors_1.UnauthorizedUserError);
        }
    }
    async createUserByNonadminToken(test, operationResults) {
        await this.userClientOperationByNonadminToken('updateUser', [{
                username: 'sdk_test_user',
                password: 'test_password'
            }]);
    }
    async createUserByApplicationToken(test, operationResults) {
        await this.userClientOperationByApplicationToken(operationResults.beforeResults[0].token, 'createUser', [{
                username: 'sdk_test_user',
                password: 'test_password'
            }]);
    }
    async updateUserByNonadminToken(test, operationResults) {
        await this.userClientOperationByNonadminToken('updateUser', [{
                username: 'sdk_test_user',
                email: 'new_email@test.com'
            }]);
    }
    async updateUserByApplicationToken(test, operationResults) {
        await this.userClientOperationByApplicationToken(operationResults.beforeResults[1].token, 'updateUser', [{
                username: 'sdk_test_user',
                email: 'new_email@test.com'
            }]);
    }
    async deleteUserByNonadminToken(test, operationResults) {
        await this.userClientOperationByNonadminToken('deleteUser', ['sdk_test_user']);
    }
    async deleteUserByApplicationToken(test, operationResults) {
        await this.userClientOperationByApplicationToken(operationResults.beforeResults[1].token, 'deleteUser', ['sdk_test_user']);
    }
    async updateUserGroupByNonadminToken(test, operationResults) {
        await this.userClientOperationByNonadminToken('updateUserGroup', ['sdk_test_user', 'sdktestgroup']);
    }
    async updateUserGroupByApplicationToken(test, operationResults) {
        await this.userClientOperationByApplicationToken(operationResults.beforeResults[0].token, 'updateUserGroup', ['sdk_test_user', 'sdktestgroup']);
    }
    async deleteUserGroupByNonadminToken(test, operationResults) {
        await this.userClientOperationByNonadminToken('deleteUserGroup', ['sdk_test_user', 'sdktestgroup']);
    }
    async deleteUserGroupByApplicationToken(test, operationResults) {
        await this.userClientOperationByApplicationToken(operationResults.beforeResults[0].token, 'deleteUserGroup', ['sdk_test_user', 'sdktestgroup']);
    }
    async updateUserGrouplistByNonadminToken(test, operationResults) {
        await this.userClientOperationByNonadminToken('updateUserGrouplist', ['sdk_test_user', ['default']]);
    }
    async updateUserGrouplistByApplicationToken(test, operationResults) {
        await this.userClientOperationByApplicationToken(operationResults.beforeResults[0].token, 'updateUserGrouplist', ['sdk_test_user', ['default']]);
    }
    async userClientOperationByApplicationToken(applicationToken, operationName, parameter) {
        const client = new clients_1.UserClient({
            token: applicationToken,
            https: true,
            rest_server_uri: clusters_json_1.default[0].rest_server_uri
        });
        try {
            await client[operationName](...parameter);
        }
        catch (err) {
            chai_1.expect(err.message).to.be.equal('Applications are not allowed to do this operation.');
            chai_1.expect(err).to.be.an.instanceof(paiUserErrors_1.ForbiddenUserError);
        }
    }
    async userClientOperationByNonadminToken(operationName, parameter) {
        const info = await this.createNonadminUserAndToken();
        const client = new clients_1.UserClient({
            token: info.token,
            https: true,
            rest_server_uri: clusters_json_1.default[0].rest_server_uri
        ...
        try {
            ...
            chai_1.expect(err.message).to.be.equal('Non-admin is not allow to do this operation.');
            chai_1.expect(err).to.be.an.instanceof(paiUserErrors_1.ForbiddenUserError);
        }
        after(async () => await this.deleteNonadminUserAndToken(info));
    }
    async createNonadminUserAndToken() {
        const openPAIClient = new clients_1.OpenPAIClient({
            token: clusters_json_1.default[0].token,
            https: true,
            rest_server_uri: clusters_json_1.default[0].rest_server_uri
        });
        try {
        ...
        return await openPAIClient.authn.basicLogin('nonadminTestUser', 'nonadminTestUser');
    }
    async deleteNonadminUserAndToken(info) {
        const openPAIClient = new clients_1.OpenPAIClient({
            token: clusters_json_1.default[0].token,
            https: true,
            rest_server_uri: clusters_json_1.default[0].rest_server_uri
