...
const azureBlobClient_1 = require("../clients/azureBlobClient");
/**
 * StorageNode class.
 */
class StorageNode {
    constructor(config) {
        this.config = config;
        const cfgDisp = this.storageConfigDispatcher(config);
        switch (cfgDisp.type) {
            case 'azureBlob': this.client = new azureBlobClient_1.AzureBlobClient(cfgDisp.data);
