...
class JobClient extends baseClient_1.OpenPAIBaseClient {
    ...
    async list(query) {
        const url = query === undefined ?
            util_1.Util.fixUrl(`${this.cluster.rest_server_uri}/api/v2/jobs`, this.cluster.https) :
            util_1.Util.fixUrl(`${this.cluster.rest_server_uri}/api/v2/jobs?${query}`, this.cluster.https);
